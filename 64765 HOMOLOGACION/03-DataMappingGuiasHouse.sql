 /*                                                                            VERSION     MODIFIEDBY          MODIFIEDDATE    HU      MODIFICATION
  1           Luchin/Patty/Juan   2025-12-19      64765   DataMapping
  GuiasHouse
  ...
  */                                                                            
  ...
  IF OBJECT_ID('tempdb..#GuiasHouseUpdate') IS NOT NULL DROP TABLE
  #GuiasHouseUpdate
  ...
  INSERT INTO #GuiasHouseUpdate (id, BilltoConsigneeId, ConsigneeId)
  SELECT
      gh.id,
      er.Id,
  ...
  WHERE er.id IS NOT NULL
    AND (gh.BilltoConsigneeId IS NULL OR gh.ConsigneeId IS NULL)
  ...
  BEGIN
      ...
      BEGIN TRY
          ...
          UPDATE gh WITH (ROWLOCK)
          SET gh.BilltoConsigneeId = ISNULL(gh.BilltoConsigneeId,
  tmp.BilltoConsigneeId),
              gh.ConsigneeId       = ISNULL(gh.ConsigneeId,
  tmp.ConsigneeId)
          ...
          INSERT INTO administracion_db..DBA_LogDepuracion
          SELECT GETDATE(),'GuiasHouse',
                 'Lote ' + CAST(@RowStart AS VARCHAR(8)) + '-' + CAST(@RowEnd
  AS VARCHAR(8)),
                 @@ROWCOUNT, GETDATE()
      ...
      BEGIN CATCH
          ...
          BEGIN
              ...
              PRINT '   -> Reintentando con BatchSize: ' + CAST(@BatchSize AS
  VARCHAR(10))
          ROLLBACK TRANSACTION
          SELECT @ErrorCount = @ErrorCount + 1
          IF @BatchSize > 500
