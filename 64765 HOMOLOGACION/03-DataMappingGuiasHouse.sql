...
IF OBJECT_ID('tempdb..#GuiasHouseUpdate') IS NOT NULL DROP TABLE #GuiasHouseUpdate
...
AND (gh.BilltoConsigneeId IS NULL OR gh.ConsigneeId IS NULL) -- Solo procesar si alguno es NULL

SELECT @TotalRecords = @@ROWCOUNT

INSERT INTO administracion_db..DBA_LogDepuracion
SELECT GETDATE(),'GuiasHouse','INICIO ACTUALIZACI�N',@TotalRecords,GETDATE()
...
BEGIN
    ...
    BEGIN TRY
        ...
        INSERT INTO administracion_db..DBA_LogDepuracion
        SELECT GETDATE(),'GuiasHouse','Lote ' + CAST(@RowStart AS VARCHAR(8)) + '-' + CAST(@RowEnd AS VARCHAR(8)),@@ROWCOUNT,GETDATE()
